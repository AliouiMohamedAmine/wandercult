...
function createCard(title, adminname, population, image, description, arnques) {
  return `
    <div class="card" >
      <img src="assets/images/neon.avif" alt="card-image5" class="card-image" onclick="goToCityInfo('${title}', '${adminname}', '${population}', '${image}', '${description}', '${arnques}')" />
      <div class="cardText">
        <h2>${title}</h2>
        <p>${adminname}</p>
      </div>
      <label class="container2">
        <input type="checkbox" class="save-checkbox" onclick="saveCity(event, '${title}', '${adminname}', '${population}', '${image}', '${description}', '${arnques}')">
...
document.getElementById(
  "description"
).innerText = `description: ${cityInfo.description}`;
